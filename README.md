...
This map is intentionally narrow. It is a local integrity demonstration, not a
production security system. The `PASS` result means the local laminar path
closed under the rules in this document. It does not mean the machine, user,
toolchain, or operating environment is trusted.

Important boundaries:

- **Groth16 setup:** the local project must generate and verify a real Groth16
  proof. That proves the local closure relation for this demo. It is not a
  public trusted-setup ceremony and it is not a production trust claim.
- **Circuit scope:** the minimal circuit constrains public field elements such
  as `final_root`, `expected_root`, `selected_hash`, and `output_hash`. It does
  not prove SHA-256 inside the circuit. The SHA-256 receipts are computed
  outside the circuit and reduced into field elements for the closure relation.
- **Hash reduction:** reducing a SHA-256 digest into the BN254 scalar field is a
  public-input bridge for the demo. It is not a claim that field reduction is a
  collision-free replacement for SHA-256. The byte-level receipts remain
  SHA-256 receipts.
- **Toolchain integrity:** the path depends on real `circom` and `snarkjs`
  binaries. A malicious or replaced toolchain can lie. Production use would need
  pinned versions, checksums or signatures, and a sandboxed build environment.
- **Receipt spoofing:** a receipt is only meaningful when it is generated by the
  step it describes and included in the parent root. Production systems should
  bind receipts to executable identity, toolchain identity, and policy version.
- **Replay:** replay must verify the same event. It must not mutate state twice.
  Any production deployment needs explicit event scope and state-root binding.
- **No hidden output:** normal failure should print only `FAIL`. A production
  implementation should also fuzz malformed inputs to confirm errors do not leak
  metadata.

If you harden this beyond the local map, the next work is straightforward:

1. audit the closure circuit for under-constrained inputs,
2. pin and verify the proof toolchain,
3. document or replace the trusted setup path,
4. fuzz the generated pipeline,
5. bind receipts to executable and policy identity,
6. add in-circuit hash constraints or a documented ZK-friendly hash bridge if
   the production threat model requires it.

The point of this repository is not to skip that work. The point is to show the
smallest closed path clearly enough that the next work is visible.
